...
- Qualitative, categorical data.
  ...
  Generally, Data is matrix/Data Table/Data frame.
...
* $x_{i,j}$ is element of an X matrix, representing the value of the `j`th variable for the
...
**Prediction ($\hat{Y}$)** predicted value by the model/algorithm.

**Training** is the way of teaching the computer the data. Machine learning algorithms are applied in this step.
                
**Training set**  is the data set used for training process. target of learning.
...
**optimization** The process of adjusting weights to produce more and more accurate guesses about the data.
...
Spam filter is binary classifier (Spam or not)
...
- Supervised Learning (We already knows the answer) / Unsupervised learning (we do not knows the answer)
    ...
    * Classification and Regression are major two objectives of supervised learning.
      ...
      * clustering, dimensionality reductions, are the main purpose of unsupervised learning.
...
- instance based learning (e.g. k-NN) / Model-based learning
     ...
     * instance based learning : No model is learned. stored "training instances" represent the "knowledge" (lazy learning) e.g) k-Nearleast neighbor.
     
     * model based learning : make a model with various parameters.
...
Step 5: calculate distance between training set and testing set and applying determined *k* to testing data for the classification.

---


Distance Calculation : *p* dimensions

 suppose that we have two vector P and Q consisting of n values.

 $$ d(A,B) = \sqrt{(a_{1}-b_{1})^2 + (a_{2}-b_{2})^2 + (a_{3}-b_{3})^2 ... (a_{p}-b_{p})^2} $$
...
* Poor Quality data
    ...
    Removal of outliers are necessary. 
...
**Bias** - differences between expected prediction and correct value which we want to predict. (accuracy in statistics)

**Variance** - variation of predicted data point. (precision in statistics)
